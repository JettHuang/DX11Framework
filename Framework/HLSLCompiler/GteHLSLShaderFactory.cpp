    ...
    ProgramDefines const& defines, unsigned int compileFlags)
{
    ...
    return CompileAndReflect(filename, source, entry, target, defines,
        compileFlags);
}
...
{
    return CompileAndReflect(name, source, entry, target, defines,
        compileFlags);
}
    ...
    for (UINT i = 0; i < numCBuffers; ++i)
    {
        ...
        if (cbDesc.Type == D3D_CT_CBUFFER)
        {
            ...
            else
            {
                ...
                {
                    shader.Insert(HLSLConstantBuffer(resDesc, j,
                        cbDesc.Size, members));
                }
            }
        }
        ...
        else if (cbDesc.Type == D3D_CT_RESOURCE_BIND_INFO)
        {
            ...
            else
            {
                for (UINT j = 0; j < resDesc.BindCount; ++j)
                {
                    shader.Insert(HLSLResourceBindInfo(resDesc, j,
                        cbDesc.Size, members));
                }
            }
        }
